...
class GardenAppearance extends StatefulWidget {
  const GardenAppearance({Key? key}): super(key:key);

  @override
  State<GardenAppearance> createState() => _GardenAppearanceState();
}
class _GardenAppearanceState extends State<GardenAppearance> {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        ...
        elevation: 4,
        title: Text('Garden Appearance', style: TextStyle(color: Colors.white)),
      ...
      body: Container(
        ...
        child: ListView(
          children: [
            settingTile(context, 'Plant Selection'),
            SizedBox(height: 15,),
            settingTile(context, 'Visual Appeal'),
            SizedBox(height: 15,),
            settingTile(context, 'Volunteer Plants'),
            SizedBox(height: 15,),
            settingTile(context, 'Invasive Species'),
          ],
        ),
      ),
    );
  ...
  List<String> getComplaintTypesForType(String type) {
    switch (type) {
      case 'Plant Selection':
        return ['Plant Selection', 'Visual Appeal','Volunteer Plants', 'Invasive Species'];
      case 'Visual Appeal':
        return ['Plant Selection', 'Visual Appeal','Volunteer Plants', 'Invasive Species'];
      case 'Volunteer Plants':
        return ['Plant Selection', 'Visual Appeal','Volunteer Plants', 'Invasive Species'];
